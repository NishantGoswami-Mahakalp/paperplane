...
export interface TTimerStartResponse extends TTimerSession {}
...
export interface TTimeReportSummary {
  date: string;
  total_minutes: number;
  worklog_count: number;
}

export interface TTimeReportResponse {
  summaries: TTimeReportSummary[];
  total_minutes: number;
...
class WorklogService extends APIService {
  ...
  async getTimeReports(
    ...
    projectId: string,
    itemId: string,
    params?: { period?: "daily" | "weekly"; start_date?: string; end_date?: string }
  ): Promise<TTimeReportResponse> {
    return this.get(`/api/workspaces/${workspaceSlug}/projects/${projectId}/items/${itemId}/worklogs/reports/`, {
      params,
    })
      .then((response) => response?.data)
      .catch((error) => {
  ...
  async exportWorklogs(
    ...
    return this.get(`/api/workspaces/${workspaceSlug}/projects/${projectId}/items/${itemId}/worklogs/export/`, {
      params,
      responseType: "blob",
    })
      .then((response) => response?.data)
